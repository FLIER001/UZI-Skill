...
- 不修改 `stage2` / `agent_analysis_validator` / `assemble_report`（这些已能消费任意来源的 `agent_analysis.json`）。
...
1. `lib/pipeline/run.py`：在 `score_from_cache(ticker)` 与
   `synthesize_and_render(ticker)` 之间插入 `maybe_run_llm_review(ticker)`。
2. `run.py` legacy 分支：在 `_stage1(...)` 与 `_stage2(...)` 之间插入同一调用。

`stage2` / `agent_analysis_validator` / `assemble_report` **不改**——它们
本就设计为消费任意来源（含非 Claude 模型）的 `agent_analysis.json`。
...
5. 各组并发（ThreadPoolExecutor max_workers=4）：
      system = stable_system_prompt()                  # 8 次调用字节级一致 → prompt cache
      user   = build_group_prompt(g, 组内 investors, dims,
                                  flagship 注入 personas/{id}.yaml 的 key_metrics/voice,
                                  panel 骨架分)
      resp   = client.chat_json(system, user)          # JSON mode
...
- **修改**：`lib/pipeline/run.py`（+1 调用）、`run.py`（legacy 分支 +1 调用）、
  `.env.example` / README / AGENTS.md / SKILL.md（文档说明新 env）
- **不改**：`stage2`、`agent_analysis_validator`、`assemble_report`、
  22 个 fetcher、score 段
